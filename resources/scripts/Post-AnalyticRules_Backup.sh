...
if [ -z "$RESOURCE_GROUP_NAME" ] || [ -z "$WORKSPACE_NAME" ]; then
    ...
    #for row in $(curl -sS https://raw.githubusercontent.com/OTRF/Blacksmith/master/templates/azure/Sentinel2Go/analytic-rules/allAnalyticRules.json | jq -r '.[] | @base64'); do
